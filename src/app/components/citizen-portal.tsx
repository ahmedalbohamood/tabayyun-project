...
export function CitizenPortal({ lang }: { lang: Lang }) {
  ...
  const [form, setForm] = useState<FormData>({
    ...
    isAnonymous: false, reportTitle: '', suspicionType: '',
    description: '', entity: '', city: '', incidentDate: '',
  });
  const [files, setFiles] = useState<UploadedFile[]>([]);
  const [isDragging, setIsDragging] = useState(false);
  const [submitted, setSubmitted] = useState(false);
  const [referenceNumber] = useState(() => `TBY-2025-${String(Math.floor(Math.random() * 900) + 900).padStart(4, '0')}`);
    ...
    return (
      <div className="min-h-screen flex items-center justify-center px-4 py-16"
        ...
        <div className="max-w-lg w-full bg-white rounded-2xl shadow-xl p-10 text-center"
          ...
          <button
            onClick={() => { setSubmitted(false); setForm({ fullName: '', idNumber: '', email: '', phone: '', isAnonymous: false, reportTitle: '', suspicionType: '', description: '', entity: '', city: '', incidentDate: '' }); setFiles([]); }}
      ...
      <form onSubmit={handleSubmit} className="max-w-4xl mx-auto px-6 pb-16 space-y-6">
          ...
          {form.isAnonymous ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              <Field label={tr.fullName} icon={<User size={14} />} error={errors.fullName}>
                <input
                  type="text" placeholder={tr.fullNamePlaceholder}
                  value={form.fullName} onChange={e => updateForm('fullName', e.target.value)}
                  className="w-full px-4 py-3 rounded-xl outline-none transition-all text-right"
                  style={{ background: '#f5f8f6', border: `1px solid ${errors.fullName ? '#c0392b' : 'rgba(0,92,46,0.15)'}`, color: '#1a2e20' }} />
              </Field>
              <Field label={tr.idNumber} icon={<IdCard size={14} />} error={errors.idNumber}>
                <input
                  type="text" placeholder={tr.idPlaceholder}
                  value={form.idNumber} onChange={e => updateForm('idNumber', e.target.value)}
                  className="w-full px-4 py-3 rounded-xl outline-none transition-all text-right"
                  style={{ background: '#f5f8f6', border: `1px solid ${errors.idNumber ? '#c0392b' : 'rgba(0,92,46,0.15)'}`, color: '#1a2e20' }} />
              </Field>
              <Field label={tr.email} icon={<Mail size={14} />} error={errors.email}>
                <input
                  type="email" placeholder="example@domain.com" dir="ltr"
                  value={form.email} onChange={e => updateForm('email', e.target.value)}
                  className="w-full px-4 py-3 rounded-xl outline-none transition-all text-left"
                  style={{ background: '#f5f8f6', border: `1px solid ${errors.email ? '#c0392b' : 'rgba(0,92,46,0.15)'}`, color: '#1a2e20' }} />
              </Field>
              <Field label={tr.phone} icon={<Phone size={14} />} error={errors.phone}>
                <input
                  type="tel" placeholder="05XXXXXXXX" dir="ltr"
                  value={form.phone} onChange={e => updateForm('phone', e.target.value)}
                  className="w-full px-4 py-3 rounded-xl outline-none transition-all text-left"
                  style={{ background: '#f5f8f6', border: `1px solid ${errors.phone ? '#c0392b' : 'rgba(0,92,46,0.15)'}`, color: '#1a2e20' }} />
              </Field>
            </div>
          )}
        </section>
        ...
        <section className="bg-white rounded-2xl p-8" style={{ border: '1px solid rgba(0,92,46,0.1)', boxShadow: '0 2px 20px rgba(0,92,46,0.05)' }}>
          ...
          <div className="space-y-5">
            <Field label={tr.reportTitle} error={errors.reportTitle}>
              <input
                type="text" placeholder={tr.reportTitlePlaceholder}
                value={form.reportTitle} onChange={e => updateForm('reportTitle', e.target.value)}
                className="w-full px-4 py-3 rounded-xl outline-none transition-all text-right"
                style={{ background: '#f5f8f6', border: `1px solid ${errors.reportTitle ? '#c0392b' : 'rgba(0,92,46,0.15)'}`, color: '#1a2e20' }} />
            </Field>

            <Field label={tr.suspicionType} error={errors.suspicionType}>
              <div className="relative">
                <select
                  value={form.suspicionType} onChange={e => updateForm('suspicionType', e.target.value)}
                  className="w-full px-4 py-3 rounded-xl outline-none transition-all text-right appearance-none cursor-pointer"
                  style={{ background: '#f5f8f6', border: `1px solid ${errors.suspicionType ? '#c0392b' : 'rgba(0,92,46,0.15)'}`, color: form.suspicionType ? '#1a2e20' : '#9ca3af' }}>
            ...
            <Field label={tr.description} error={errors.description}>
              <textarea
                ...
                className="w-full px-4 py-3 rounded-xl outline-none transition-all text-right resize-none"
                style={{ background: '#f5f8f6', border: `1px solid ${errors.description ? '#c0392b' : 'rgba(0,92,46,0.15)'}`, color: '#1a2e20', lineHeight: 1.8 }} />
        ...
        <section className="bg-white rounded-2xl p-8" style={{ border: '1px solid rgba(0,92,46,0.1)', boxShadow: '0 2px 20px rgba(0,92,46,0.05)' }}>
          ...
          <div
            ...
            <input
              ref={fileInputRef} type="file" multiple accept="image/*,application/pdf,video/*,.doc,.docx,.xls,.xlsx"
              className="hidden" onChange={e => e.target.files && addFiles(Array.from(e.target.files))} />
            <div className="w-14 h-14 rounded-2xl flex items-center justify-center mx-auto mb-4"
              style={{ background: isDragging ? '#e8f0eb' : '#f0f4f1' }}>
            ...
            <div className="mt-4 space-y-2">
              {files.map(f => (
                <div key={f.id} className="flex items-center justify-between px-4 py-3 rounded-xl"
                  style={{ background: '#f5f8f6', border: '1px solid rgba(0,92,46,0.1)' }}>
                  <button onClick={() => setFiles(prev => prev.filter(x => x.id !== f.id))} className="p-1 rounded-lg hover:bg-red-50 transition-colors">
        ...
        <section className="bg-white rounded-2xl p-8" style={{ border: '1px solid rgba(0,92,46,0.1)', boxShadow: '0 2px 20px rgba(0,92,46,0.05)' }}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
            <Field label={tr.govEntity} icon={<Building2 size={14} />} error={errors.entity}>
              <div className="relative">
                <select
                  value={form.entity} onChange={e => updateForm('entity', e.target.value)}
                  className="w-full px-4 py-3 rounded-xl outline-none transition-all text-right appearance-none cursor-pointer"
                  style={{ background: '#f5f8f6', border: `1px solid ${errors.entity ? '#c0392b' : 'rgba(0,92,46,0.15)'}`, color: form.entity ? '#1a2e20' : '#9ca3af' }}>
            ...
            <Field label={tr.city} icon={<MapPin size={14} />} error={errors.city}>
              <div className="relative">
                <select
                  ...
                  className="w-full px-4 py-3 rounded-xl outline-none transition-all text-right appearance-none cursor-pointer"
                  style={{ background: '#f5f8f6', border: `1px solid ${errors.city ? '#c0392b' : 'rgba(0,92,46,0.15)'}`, color: form.city ? '#1a2e20' : '#9ca3af' }}>
            ...
            <Field label={tr.incidentDate} icon={<Calendar size={14} />} error={errors.incidentDate}>
              <input
                type="date" value={form.incidentDate} onChange={e => updateForm('incidentDate', e.target.value)}
                className="w-full px-4 py-3 rounded-xl outline-none transition-all"
                style={{ background: '#f5f8f6', border: `1px solid ${errors.incidentDate ? '#c0392b' : 'rgba(0,92,46,0.15)'}`, color: form.incidentDate ? '#1a2e20' : '#9ca3af', direction: 'ltr' }} />
            </Field>
          </div>
        </section>
        ...
        <button
          type="submit"
          className="w-full py-4 rounded-2xl text-white text-lg transition-all hover:opacity-95 hover:shadow-xl active:scale-[0.99]"
